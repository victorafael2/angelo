...
if (isset($_POST['vt_nome']) && !empty($_POST['vt_nome'])) {
    $nome_sistema = $_POST['vt_nome'];

} elseif (isset($_POST['vt_nome_modal']) && !empty($_POST['vt_nome_modal'])) {
    $nome_sistema = $_POST['vt_nome_modal'];

} else {
    // Lide com o caso em que nenhum dos campos está definido
    $response = array('status' => 'error', 'message' => 'O nome do VT não foi fornecido.');
...
if (isset($_POST['id_modal']) && !empty($_POST['id_modal'])) {
    ...
    $sql = "SELECT * FROM aux_vt WHERE vt_nome = '$nome_sistema' AND  id_vt != $id_modal";
} else {
    // Consulta SQL para verificar se o nome da área já existe
    $sql = "SELECT * FROM aux_vt WHERE vt_nome = '$nome_sistema' ";
}
...
if ($result->num_rows > 0) {
    ...
    $response = array('status' => 'error', 'message' => 'O nome do VT já existe no banco de dados.');
} else {
    // O nome da área não existe, retorne uma mensagem de sucesso
    $response = array('status' => 'success', 'message' => 'Nome do VT disponível.');
}
