...
if (isset($_POST['cargo_nome']) && !empty($_POST['cargo_nome'])) {
    $nome_sistema = $_POST['cargo_nome'];
...
if (isset($_POST['id_modal']) && !empty($_POST['id_modal'])) {
    ...
    $sql = "SELECT * FROM aux_cargos WHERE cargo_nome = '$nome_sistema' AND id_cargo != $id_modal";
} else {
    // Consulta SQL para verificar se o nome da área já existe
    $sql = "SELECT * FROM aux_cargos WHERE cargo_nome = '$nome_sistema'";
}
