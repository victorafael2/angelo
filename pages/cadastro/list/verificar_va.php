...
if (isset($_POST['vr_nome']) && !empty($_POST['vr_nome'])) {
    $nome_sistema = $_POST['vr_nome'];
...
if (isset($_POST['id_modal']) && !empty($_POST['id_modal'])) {
    ...
    $sql = "SELECT * FROM aux_vr WHERE vr_nome = '$nome_sistema' AND  id_vr != $id_modal";
} else {
    // Consulta SQL para verificar se o nome da área já existe
    $sql = "SELECT * FROM aux_vr WHERE vr_nome = '$nome_sistema' ";
}
