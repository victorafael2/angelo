<style>
    ...
    <div style="display: flex; justify-content: space-between;" class="mb-2">
        <h3 style="margin: 0;">Plano Odontológico</h3>
        <button id="adicionarBtn" class="btn btn-sm btn-phoenix-success">Adicionar Plano Odontológico</button>
    </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form id="aux_ali_form">
                                ...
                                <button type="submit" class="btn btn-primary mt-2">Cadastrar Plano Odontológico</button>
                            </form>
                        </div>
                    </div>
                </div>
...
$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                data.forEach(function(item) {
                    ...
                    tableData += "<td class='amount align-middle white-space-nowrap fw-bold ps-4 text-900 py-0'><span class='status-icon' data-id='" + item.id_po +
                        "'>";
    ...
    document.getElementById("aux_ali_form").addEventListener("submit", function(event) {
        ...
        $.ajax({
            url: 'pages/cadastro/list/verificar_po.php',
            type: 'POST',
            data: {
                ps_nome: ps_nome
            ...
            success: function(response) {
                if (response.status === 'error') {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Erro!',
                        text: 'O nome do Plano Odontológico já está em uso. Escolha outro nome.',
                        confirmButtonText: 'OK'
                    });
                } else {
        ...
        $.ajax({
            url: 'pages/config/insert/salve_plano_odonto.php',
            type: 'POST',
            data: {
                ps_nome: ps_nome,
                ps_valor: ps_valor,
