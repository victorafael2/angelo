<style>
    ...
    <div style="display: flex; justify-content: space-between;" class="mb-2">
        <h3 style="margin: 0;">Outros Benefícios</h3>
        <button id="adicionarBtn" class="btn btn-sm btn-phoenix-success">Adicionar Benefícios</button>
    </div>
                ...
                <div class="col-md-12 mb-2" id="cadastro">
                    <div class="card">
                        <div class="card-header">
                            Formulário de Outros Benefícios
                        ...
                        <div class="card-body">
                            <form id="aux_ali_form">
                                ...
                                <button type="submit" class="btn btn-primary mt-2">Cadastrar Beneficio</button>
                            </form>
                        </div>
                    </div>
                </div>
                                    ...
                                    <table class="table table-striped table-sm fs--1 mb-0">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="sort border-top " data-sort="vt_desc">Descrição</th>
...
$(document).ready(function() {
    ...
    function loadItems() {
        $.ajax({
            url: 'pages/config/insert/get_outros_beneficios.php',
            type: 'GET',
            dataType: 'json',
            success: function(data) {
                var tableData = "";
                data.forEach(function(item) {
                    tableData += "<tr><td>" + item.id_op + "</td><td>" + item.op_nome +
                        "</td><td>" + item.op_desc +
                        "</td><td>" + item.op_valor + "</td>";

                        tableData += "<td class='amount align-middle white-space-nowrap fw-bold ps-4 text-900 py-0'><span class='status-icon' data-id='" + item.id_op +
                        "'>";
                        ...
                        tableData += "</span></td>";
                        tableData += "<td><button class='delete-btn btn btn-danger btn-sm apagar_va' data-id='" +
                        item.id_op + "'>Excluir</button></td></tr></tr>";
                });
    ...
    document.getElementById("aux_ali_form").addEventListener("submit", function(event) {
        ...
        var habilitado = document.getElementById("habilitado").value;
                ...
                $.ajax({
            url: 'pages/cadastro/list/verificar_op.php',
            type: 'POST',
            data: {
                nome: nome
            ...
            success: function(response) {
                if (response.status === 'error') {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Erro!',
                        text: 'O nome do Benefício já está em uso. Escolha outro nome.',
                        confirmButtonText: 'OK'
                    });
                } else {
...
$(document).on("click", ".status-icon", function() {
    ...
    $.ajax({
        type: 'POST',
        url: 'pages/cadastro/update/update_outros.php',
        data: {
            id: id
        },
        dataType: 'json',
